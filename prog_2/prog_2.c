//following is the program to convert an infix mathematical expression to postfix and evaluate it.
...
void infix_to_postfix(char* infix, char* postfix)
{
    ...
    for(int i = 0; infix[i] != '\0'; i++)
    {
        ...
        {
            while(top_op != -1 && prec(op_stack[top_op]) >= prec(ch)&&
                  !is_right_assoc(ch)){
                postfix[k++] = pop_op();
            }
            push_op(ch);
